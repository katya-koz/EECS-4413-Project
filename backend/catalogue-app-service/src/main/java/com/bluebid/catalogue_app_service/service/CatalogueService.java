...
public class CatalogueService {
	...
	public Page<CatalogueItem> getAllAvailableItems(Pageable page) {
        //return _catalogueRepository.findByIsActive(true);
		return _catalogueRepository.findByAuctionEndDateAfter(LocalDateTime.now(), page);
    }

    public Page<CatalogueItem> searchAvailableItems(String keyword, Pageable page) {
    	// we only want items that are still up for auction
    	
        return _catalogueRepository.findByKeywordQueryAndNotEnded( keyword,LocalDateTime.now(), page); // case insensitive search
